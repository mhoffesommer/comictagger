...
class ComicInfo(Metadata):
    settings_section: str = "comicinfo"
    ...
    def __init__(self, settings: Mapping[str, any]):
        self.has_comet = None
        self.comicinfo_filename = "ComicInfo.xml"

    def archiveSupportsMetadata(self, archive: Archiver) -> bool:
        return True  # currently have no way to identify that an archive can store arbitrary files

    def archiveHasMetadata(self, archive: Archiver) -> bool:
        return self.archiveSupportsMetadata(archive) and self.comicinfo_filename in self.archiver.getFilenameList()
    ...
    def metadataFromArchive(self, archive: Archiver) -> GenericMetadata:
        if self.archiveHasMetadata(archive):
            return self.metadataFromString(archive.readFile(self.comicinfo_filename))
        return GenericMetadata

    def metadataToArchive(self, metadata: GenericMetadata, archive: Archiver) -> bool:
        if self.archiveSupportsMetadata(archive):
            return archive.writeFile(self.comicinfo_filename, self.stringFromMetadata(metadata))
        return False
